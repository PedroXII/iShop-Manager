// pages/api/fornecedor.js
export default async function handler(req, res) {
    const BASE_URL = "https://parseapi.back4app.com/classes/Fornecedor";
    ...
    try {
      ...
      if (req.method === "GET") {
        ...
        // Filtro para buscar apenas os fornecedores da loja do usuário logado
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.message || "Erro ao carregar fornecedores.");
        }
      ...
      // Adicionar Fornecedor (POST)
        ...
        const { nome, fornecimento, observacao, loja } = req.body;
      ...
      // Editar Fornecedor (PUT)
        ...
        if (!response.ok) {
          const errorData = await response.json();
          console.error("Erro ao atualizar fornecedor:", errorData);
      ...
      // Excluir Fornecedor (DELETE)
        ...
        if (!response.ok) {
          const errorData = await response.json();
          console.error("Erro ao excluir fornecedor:", errorData);
          throw new Error(errorData.message || "Erro ao excluir fornecedor.");
        }
  
        console.log("Fornecedor excluído:", objectId);
